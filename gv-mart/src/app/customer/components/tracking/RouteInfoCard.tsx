...
function formatDistance(t: (key: string, opts?: Record<string, unknown>) => string, distanceKm: number) {
  if (distanceKm < 1) return t("customerApp.tracking.routeCard.distanceValueMeters", { meters: Math.max(10, Math.round(distanceKm * 1000)) })
  return t("customerApp.tracking.routeCard.distanceValue", { distance: distanceKm.toFixed(1) })
}
    ...
    <div className="rounded-[20px] border border-border bg-surface px-5 py-4.5 shadow-[0_10px_30px_-14px_rgba(26,26,26,.16)]">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        ...
        <div className="border-x border-border">
          ...
          <p className="gv-tnum mt-1 text-lg font-extrabold text-text">{distanceKm != null ? formatDistance(t, distanceKm) : "—"}</p>
        </div>
        <div>
